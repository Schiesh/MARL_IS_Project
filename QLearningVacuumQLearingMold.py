...
CLEAN_PENALTY = 10
SURVIVED_REWARD = 25
epsilon = 0.0 # Start at 0.9, Once trained change value to 0.0
EPS_DECAY = 0.9998
SHOW_EVERY = 3000
...
for episode in range(HM_EPISODES):
    ...
    for i in range(200):
        ...
        dirt_action = np.argmin(vacuum_q_table[obs])
        ...
        vacuum_q_table[obs][dirt_action] = dirt_new_q
        ...
        dirt_episode_reward += dirt_reward
        if reward == CLEAN_REWARD or reward == -OUT_OF_BOUNDS:
            cv2.destroyAllWindows()
            break
        
    episode_rewards.append(episode_reward)
    dirt_episode_rewards.append(dirt_episode_reward)
    epsilon *= EPS_DECAY

moving_avg = np.convolve(episode_rewards, np.ones((SHOW_EVERY,)) / SHOW_EVERY, mode="valid")
dirt_moving_avg = np.convolve(dirt_episode_rewards, np.ones((SHOW_EVERY,)) / SHOW_EVERY, mode="valid")
